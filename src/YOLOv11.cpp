...
void YOLOv11::build(std::string onnxPath, nvinfer1::ILogger& logger)
{
    ...
    if (isFP16) { config->setFlag(BuilderFlag::kFP16); }
    config->setMemoryPoolLimit(nvinfer1::MemoryPoolType::kWORKSPACE, 16L * 1024 * 1024 * 1024); // 16GB
    config->setFlag(nvinfer1::BuilderFlag::kDEBUG);    // Debugging mode

    std::cout << "Parsing ONNX model..." << std::endl;
    nvonnxparser::IParser* parser = nvonnxparser::createParser(*network, logger);
    bool parsed = parser->parseFromFile(onnxPath.c_str(), static_cast<int>(nvinfer1::ILogger::Severity::kVERBOSE));
    if (!parsed) {
