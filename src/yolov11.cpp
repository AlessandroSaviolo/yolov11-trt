...
// Constructors for the YOLOv11 class
YOLOv11::YOLOv11() {}

YOLOv11::YOLOv11(string model_path, nvinfer1::ILogger& logger, float conf_threshold, float nms_threshold)
{
    // Check if the model path does not contain ".onnx"
    if (model_path.find(".onnx") == std::string::npos)
    {
        // Initialize the engine from a serialized engine file
        init(model_path, logger, conf_threshold, nms_threshold);
    }
    else
    {
                   ...
                   float conf_threshold, float nms_threshold)
{
    ...
    if (warmup) {
        ...
        printf("[YOLOv11] Model warmup 10 times\n");
    }
...
void YOLOv11::postprocess(vector<Detection>& output)
{
    ...
    dnn::NMSBoxes(boxes, confidences, _conf_threshold, _nms_threshold, nms_result);
...
// Postprocess the inference output to extract detections
void YOLOv11::postprocess(vector<Detection>& output, Point& target_coord, Mat& image)
{
    // Asynchronously copy output from GPU to CPU
    CUDA_CHECK(cudaMemcpyAsync(cpu_output_buffer, gpu_buffers[1], num_detections * detection_attribute_size * sizeof(float), cudaMemcpyDeviceToHost, stream));
    ...
    // Draw bounding boxes and retrieve target center coordinate
    draw(output, target_coord, image);
}
...
// Draw bounding boxes and labels on the image based on detections
void YOLOv11::draw(const vector<Detection>& output, Point& target_coord, Mat& image)
{
    // Calculate the scaling ratios between input and original image dimensions
    const float ratio_h = input_h / (float)image.rows;
    const float ratio_w = input_w / (float)image.cols;
    ...
    for (int i = 0; i < output.size(); i++)
    {
        ...
        if (i == 0) {
            ...
            target_coord.x = box.x + box.width / 2;
            target_coord.y = box.y + box.height / 2;

            // Plot target coordinate
            cv::circle(image, target_coord, 3, cv::Scalar(0, 255, 0), -1);

            // Draw the bounding box on the image
            rectangle(image, Point(box.x, box.y), Point(box.x + box.width, box.y + box.height), cv::Scalar(0, 255, 0), 3);
        }
        else {
            // Draw the bounding box on the image
            rectangle(image, Point(box.x, box.y), Point(box.x + box.width, box.y + box.height), cv::Scalar(0, 0, 255), 3);
        }
