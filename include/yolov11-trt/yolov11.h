...
public:
    ...
    /**
     * @brief Constructors to initialize the YOLOv11 object.
     *
     ...
     */
    YOLOv11();
    YOLOv11(string model_path, nvinfer1::ILogger& logger, float conf_threshold, float nms_threshold);
     ...
     * @param logger Reference to a TensorRT logger for error reporting.
     */
    void init(std::string engine_path, nvinfer1::ILogger& logger, float conf_threshold, float nms_threshold);
    ...
    void postprocess(vector<Detection>& output, Point& target_coord, Mat& image);
    ...
    void draw(const vector<Detection>& output, Point& target_coord, Mat& image);
...
private:
    ...
    float _conf_threshold = 0.3f; //!< Confidence threshold for filtering detections.
    float _nms_threshold = 0.4f; //!< Non-Maximum Suppression (NMS) threshold for filtering overlapping boxes.
